...
export const LimelightNav = ({
  ...
  useLayoutEffect(() => {
    ...
    if (limelight && activeItem) {
      ...
      const lightWidth = limelight.offsetWidth;
      
      // FAZA 2: Scriere separată (Write via RAF) pentru a preveni Forced Reflow (Layout Thrashing)
      requestAnimationFrame(() => {
        const translateX = itemLeft + itemWidth / 2 - lightWidth / 2;
        limelight.style.transform = `translateX(${translateX}px)`;
  ...
  return (
    <nav className={`relative inline-flex items-center h-16 rounded-lg bg-card text-foreground border px-2 ${className}`}>
      {items.map(({ id, icon, label, onClick }, index) => (
          ...
          >
            {cloneElement(icon, {
              className: `w-6 h-6 transition-all duration-100 ease-in-out ${
                activeIndex === index ? 'opacity-100 text-[#81ecff]' : 'opacity-40'
      ...
      <div 
        ref={limelightRef}
        className={`absolute top-0 left-0 z-10 w-11 h-[5px] rounded-full bg-[#81ecff] shadow-[0_20px_15px_rgba(129,236,255,0.5)] will-change-transform ${
          isReady ? 'transition-transform duration-500 ease-in-out' : ''
        } ${limelightClassName}`}
        style={{ transform: 'translateX(-999px)' }}
      >
        <div className="absolute left-[-30%] top-[5px] w-[160%] h-14 [clip-path:polygon(5%_100%,25%_0,75%_0,95%_100%)] bg-gradient-to-b from-[#81ecff]/30 to-transparent pointer-events-none" />
      </div>
    </nav>
  );
};
